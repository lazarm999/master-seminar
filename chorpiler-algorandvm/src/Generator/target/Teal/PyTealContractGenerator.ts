...
export class PyTealContractGenerator implements TemplateEngine {
    ...
    async compile(_iNet: InteractionNet, _template?: string): Promise<{ target: string, encoding: ProcessEncoding }> {
        ...
        console.log('PyTealContractGenerator compile function')

        const gen = ProcessGenerator.generate(iNet);

        return {
            target: Mustache.render(template, gen.options),
            encoding: gen.encoding
        };
    }

    async getTemplate(): Promise<string> {
        return (await readFile(path.join(__dirname, '..', '..', 'templates/ProcessExecutionTemplate.py'))).toString();
    }
